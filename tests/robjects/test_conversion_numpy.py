...
@pytest.mark.skipif(not has_numpy, 'package numpy cannot be imported')
class TestNumpyConversions(object):

    def test_activate(self):
        rpyn.deactivate()
        #FIXME: is the following still making sense ?
        assert rpyn.py2ri != conversion.py2ri
        l = len(conversion.py2ri.registry)
        k = set(conversion.py2ri.registry.keys())
        rpyn.activate()
        assert len(conversion.py2ri.registry) > l
        rpyn.deactivate()
        assert len(conversion.py2ri.registry) == l
        assert set(conversion.py2ri.registry.keys()) == k
    ...
    def test_vector_unicode_character(self):
        # TODO: still relevant in Python 3 ?
        l = [u"a", u"b", u"c"]
