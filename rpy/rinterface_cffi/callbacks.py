...
def _consoleread(prompt, buf, n, addtohistory):
    try:
        reply = consoleread(_rinterface.ffi.string(prompt).decode('ASCII'))
...
def _consolewrite_ex(buf, n, otype):
    s = _rinterface.ffi.string(buf, maxlen=n).decode('ASCII')
    if otype == 0:
...
def _showmessage(buf):
    s = _rinterface.ffi.string(buf).decode('ASCII')
    showmessage(s)
...
def _cleanup(saveact, status, runlast):
     choosefile(saveact, status, runlast)
